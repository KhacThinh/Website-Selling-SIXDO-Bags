CREATE
DATABASE DATN_CUAHANGTUIXACHSixDoo
GO
...
ALTER
COLUMN chat_lieu NVARCHAR(200);

ALTER TABLE san_pham
ALTER
COLUMN xuat_xu NVARCHAR(100);

ALTER TABLE san_pham
ALTER
COLUMN mo_ta NVARCHAR(MAX);

ALTER TABLE san_pham
ALTER
COLUMN anh NVARCHAR(MAX);
...
(
    ...
    ten_day_du_cua_hang NVARCHAR(255),
    ten_cua_hang        NVARCHAR(255),
    dia_chi             NVARCHAR(max),
    hinh_anh_logo       NVARCHAR(max),
    sdt                 NVARCHAR(20),
    email               NVARCHAR(255),
    toa_do_map_x        VARCHAR(255),
    toa_do_map_y        VARCHAR(255)
);

CREATE TABLE slide_show
(
    id                    INT IDENTITY(1,1) PRIMARY KEY,
    text_collection_title VARCHAR(200) NOT NULL,
    text_new_arrivals     TEXT,
    url_hinh_anh          VARCHAR(255),
    link_chuyen_trang     VARCHAR(255),
    trang_thai            INT
);
...
DECLARE
@start_date datetime = '2024-03-10';  -- Thay đổi giá trị theo ngày bạn muốn
DECLARE
@end_date datetime = '2030-01-01';    -- Thay đổi giá trị theo ngày bạn muốn
...
from khuyen_mai
    INSERT
INTO san_pham (ma_san_pham, id_thoi_gian_bao_hanh, id_thuong_hieu, id_danh_muc, id_doi_tuong_su_dung, ten, anh,
               product_url, kich_thuoc, khoi_luong, chat_lieu, xuat_xu, mo_ta, trang_thai)
...
select *
from san_pham
where khoi_luong between 0.4 and 1
select *
from san_pham
where trang_thai = 1
  and ten like N'%phẩm 2%'
...
where ctsp.ma_ctsp like '%ctsp0001%'
   or sp.ten like '%ctsp0001%'
   or ms.ten like '%ctsp0001%'
...
from chi_tiet_san_pham as ctsp
         ...
         join
     mau_sac as ms on ctsp.id_mau_sac = ms.id
         join doi_tuong_su_dung as dtsd on dtsd.id = sp.id_doi_tuong_su_dung <<<<<<< HEAD
where sp.chat_lieu like N'%%'
  and ms.ten like N'%%'
  and dtsd.ten like '%%'
    =======
where sp.chat_lieu = N'Nhựa'
  and ms.ten = N''
  and dtsd.ten = ''
    >>>>>>
...
WHERE (sp.chat_lieu = N'Nhựa')
   or (ms.ten = N'Trắng' OR ms.ten = N'')
   or (dtsd.ten = 'Nam' OR dtsd.ten = N'')


select *
from hoa_don as hd
         join chi_tiet_hoa_don as cthd on hd.id = cthd.id_hoa_don
select *
from doi_tuong_su_dung
...
from chi_tiet_san_pham as ctsp
         ...
         join
     mau_sac as ms on ctsp.id_mau_sac = ms.id
         join thuong_hieu as th on sp.id_thuong_hieu = th.id
         join doi_tuong_su_dung as dtsd on sp.id_doi_tuong_su_dung = dtsd.id
where sp.chat_lieu like N'%Titan%'
  and dtsd.ten like N'%%'
  and ms.ten like N'%%'
  and th.ten like N'%%'
...
SELECT TOP 5
    sp.ten AS 'Tên sản phẩm', SUM(cthd.so_luong) AS 'Số lượng đã bán'
...
@Year INT = 2024;
DECLARE
@Month INT = 3;-- Thay đổi năm tại đây
SELECT
...
ON hd.id = cthd.id_hoa_don
WHERE
    YEAR (hd.thoi_gian_tao) = @Year
   OR
...
SELECT sp.ten as N'Tên Sản Phẩm', cthd.so_luong as N'Số Lượng Mua'
FROM chi_tiet_hoa_don cthd
         JOIN
     chi_tiet_san_pham cts ON cthd.id_ctsp = cts.id
         JOIN
     san_pham sp ON cts.id_san_pham = sp.id
where sp.ten like N'%Gucci%'
order by sp.ten



SELECT sp.ten AS 'Tên sản phẩm', SUM(cthd.so_luong) AS 'Số lượng đã bán', FORMAT(SUM(cthd.so_luong * cts.gia_ban), '#,###') AS 'Số Tiền Bán', FORMAT(
            SUM(cthd.so_luong * cts.gia_ban) - SUM(cts.gia_nhap * cthd.so_luong), '#,###') AS 'Tiền Lãi'
...
select *
from san_pham
         join chi_tiet_san_pham on san_pham.id = chi_tiet_san_pham.id_san_pham
...
from gio_hang
delete
khach_hang where id =23
delete
gio_hang where id =11
delete
tai_khoan where id =25
